...
name1<-paste('basic_mean_',index,'.txt', sep = '')
name2<-paste('basic_median_',index,'.txt', sep = '')
beta<-c(int, a)
...
y_asym<-matrix(rep(mu,N),N,T,byrow=TRUE)+t(lambda%*%t(u))+e

#### random sample which person is to have outlier
which_out<-sample.int(N, N*p)
for(i in which_out){
  #### random sample which time point is the outlier, only 1 outlier per person
  
  which_session<-sample(c(1:T),how_many)
  y_asym[i,which_session]<-rnorm(how_many,0,sqrt(varE))
}
